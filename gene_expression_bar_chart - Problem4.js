...
/**
 * Assume that the gene_expression.json file can contain different genes 
 * (for testing purposes TEST_gene_expression.json has "PTEN" and "PTEN_TEST" genes)
 * Parse the data from the TEST_gene_expression.json and
 * plot the median expression value only for the gene specified in the chart setting below
 */
...
(function () {
  ...
  const containerId = "#problem4";
  const chartTitle = "Problem 4"
  const sort_options = {
  ...
  const geneSymbol = "PTEN"; //"PTEN_TEST";
  ...
  Promise.all([
    d3.json("TEST_gene_expression.json"),
    ...
    data = _
      .chain(exprArr)
      .filter(function(o) { return o.geneSymbol === geneSymbol; })  // assume that we need to visualize expression of a specific gene
